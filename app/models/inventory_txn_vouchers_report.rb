class InventoryTxnVouchersReport < ActiveType::Object
  ...
  def self.inventory_internal_transfer_vouchers_line_items(options = {})
    inventory_txn_ids = InventoryTxn.where("primary_location_id IN (#{GlobalSettings.current_bookstall_id}, #{GlobalSettings.current_stores_id}) OR secondary_location_id IN (#{GlobalSettings.current_bookstall_id}, #{GlobalSettings.current_stores_id})").pluck('DISTINCT id')
    product_ids = InventoryTxnLineItem.where(inventory_txn_id: inventory_txn_ids).pluck('DISTINCT product_id')
    product_details = Product.product_details_by_ids(product_ids)
    ...
    InventoryTxnLineItem.includes(inventory_txn: [:created_by, :primary_location, :secondary_entity, :secondary_location]).where(inventory_txn_id: inventory_txn_ids).order("inventory_txns.voucher_date, inventory_txns.number").references(inventory_txn: [:created_by, :primary_location, :secondary_entity, :secondary_location]).find_each do |line_item|
      ...
      myHash = Hash.new
      myHash['VoucherDate'] = line_item.inventory_txn.voucher_date.strftime('%d/%m/%Y')
      myHash['VoucherName'] = line_item.inventory_txn.number
      myHash['VoucherType'] = line_item.inventory_txn.type
      ...
      myHash['Location'] = line_item.inventory_txn.primary_location_name
      myHash['QuantitySent'] = line_item.quantity_out
      myHash['SecondaryLocation'] = line_item.inventory_txn.secondary_location_name || line_item.inventory_txn.secondary_entity_name
      myHash['QuantityReceived'] = line_item.quantity_in
      myHash['CreatedBy'] = line_item.inventory_txn.created_by_name
