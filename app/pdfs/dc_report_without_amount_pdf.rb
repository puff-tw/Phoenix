class DcReportWithoutAmountPdf < Prawn::Document
  ...
  def line_items
    ...
    @result.each do |item|
      ...
      text 'Delivery Challan', size: 11, style: :bold, align: :center
      ...
      text 'DC-CHE-'+j.to_s, size: 8, style: :bold, align: :right
      text Time.zone.now.strftime('%d/%m/%Y'), size: 8, style: :bold, align: :right


      result = [['S.No', "Code","Type", "Description", "Quantity"]]
      i = 1
      item.each do |item1|
        #price = (Product.select('selling_price').where(:sku => item1['Sku'].to_i).limit(1).pluck(:selling_price)[0].to_i)
        result += [[i,item1['Sku'], item1['PCat'], item1['ProductName'] ,item1['AvailableStock'].abs]]
        i=i+1
      end
