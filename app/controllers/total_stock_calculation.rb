class TotalStockCalculation < ActiveType::Object
  ...
  def self.calculate_sales(options = {}, filter_params={})
    ...
    bus_ent_locs.keys.each do |loc| # Locations already in sorted order
      master[loc].keys.sort.each do |product|
        ...
        if (available_stock.to_i < limit.to_i)
          productmap = Hash.new
          ...
          productmap['PosSales'] = master[loc][product]['pos_sales']
  ...
  def self.calculate_sales_without_limit(options = {}, filter_params={})
  ...
  def self.locationwise_stock_summary(options = {}, filter_params={})
    ...
    to_date = filter_params[:to_date]
    location_id = filter_params[:location_id]
    sku = filter_params[:sku]
    quantity = filter_params[:quantity]
    ...
    bus_ent_locs.keys.each do |loc| # Locations already in sorted order
      master[loc].keys.sort.each do |product|
        ...
        if (product == sku)
          ...
          available_stock -= quantity.to_i
          if (available_stock.to_i > 0)
            availabe = true
          end

          @jtable = {'sku' => sku, 'available' => availabe}
          return @jtable

          #format.json { render json: available_stock, status: :ok }
          #render json: available_stock
        end
